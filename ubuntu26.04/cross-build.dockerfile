...
# libtorrent 1.2.x still uses older Boost.Asio APIs removed from newer Boost.
# Keep Ubuntu 26.04 builds on Boost 1.83 for tools, headers, and runtime libs.
...
RUN \
    ...
    apt-get install -yqq --no-install-recommends \
        ...
        libboost1.83-dev:${ARCH} \
        libboost-python1.83-dev:${ARCH} \
        libboost-system1.83-dev:${ARCH} \
        libssl-dev:${ARCH}
